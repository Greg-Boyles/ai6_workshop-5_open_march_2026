...
The pipeline is slow because MaxConcurrency is set to 2, meaning only 2 tickets are processed in parallel at a time. With 40 tickets queued, the batch takes a long time to complete — not because each ticket is slow, but because the throughput is capped.
...
2. **What specific metric or log proves it?** CloudWatch ConcurrentExecutions metric shows a flat line at 2. The Step Functions Map state has MaxConcurrency=2. 40 items are queued but only 2 process at a time, creating a serial bottleneck.
...
| Data / Input | No validation errors. All 40 tickets process successfully — they just process slowly in aggregate. |
...
| Per-item p95 duration | ~250-350ms | ~250-350ms |
...
- **What happened:** A batch of 40 support tickets was submitted to the ML pipeline. The batch took approximately 15-20 seconds to complete, well above the expected ~4-7 seconds. No individual ticket failed, but overall throughput was unacceptably slow.
- **Evidence:** CloudWatch ConcurrentExecutions metric capped at 2. Step Functions Map state shows MaxConcurrency=2. Per-request embed duration p95 is ~300ms (normal). Batch duration is ~18 seconds for 40 items.
...
- **Result after fix:** Batch duration dropped from ~18 seconds to ~5 seconds. ConcurrentExecutions rose from 2 to 10. Per-item duration remained unchanged, confirming the fix addressed the correct bottleneck.

---

## Extension — Next Bottleneck at MAX_CONCURRENCY=40

If you set MAX_CONCURRENCY to 40, the next bottleneck would likely be:

1. **Cold starts:** Spinning up 40 Lambda execution environments simultaneously causes a burst of cold starts. Each cold start adds 500ms-2s of initialization time, temporarily increasing p95 duration.
...
3. **Downstream throttling:** If the embed model API has its own rate limit, 40 simultaneous requests might trigger throttling from the model provider.
